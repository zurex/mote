...
import { bufferToReadable, bufferToStream, streamToBuffer, streamToBufferReadableStream, VSBuffer, VSBufferReadable, VSBufferReadableStream } from 'mote/base/common/buffer';
import { DisposableStore } from 'mote/base/common/lifecycle';
import { FileAccess, Schemas } from 'mote/base/common/network';
import { basename, dirname, join, posix } from 'mote/base/common/path';
...
flakySuite('Disk File Service', function () {
	...
	setup(async () => {
		...
		const sourceDir = FileAccess.asFileUri('mote/platform/files/test/node/fixtures/service').fsPath;
	...
	test('resolve - file', async () => {
		const resource = FileAccess.asFileUri('mote/platform/files/test/node/fixtures/resolver/index.html');
	...
	test('resolve - directory', async () => {
		...
		const resource = FileAccess.asFileUri('mote/platform/files/test/node/fixtures/resolver');
	...
	test('resolve - directory - with metadata', async () => {
		...
		const result = await service.resolve(FileAccess.asFileUri('mote/platform/files/test/node/fixtures/resolver'), { resolveMetadata: true });
	...
	test('resolve - directory - resolveTo single directory', async () => {
		const resolverFixturesPath = FileAccess.asFileUri('mote/platform/files/test/node/fixtures/resolver').fsPath;
	...
	test('resolve directory - resolveSingleChildFolders', async () => {
		const resolverFixturesPath = FileAccess.asFileUri('mote/platform/files/test/node/fixtures/resolver/other').fsPath;
