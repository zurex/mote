...
import { getNextTickChannel, ProxyChannel } from 'mote/base/parts/ipc/common/ipc';
import { Client } from 'mote/base/parts/ipc/node/ipc.cp';
import { AbstractUniversalWatcherClient, IDiskFileChange, ILogMessage, IUniversalWatcher } from 'mote/platform/files/common/watcher';
	...
	protected override createWatcher(disposables: DisposableStore): IUniversalWatcher {
		...
		const client = disposables.add(new Client(
			FileAccess.asFileUri('bootstrap-fork').fsPath,
			{
				...
				env: {
					VSCODE_AMD_ENTRYPOINT: 'mote/platform/files/node/watcher/watcherMain',
