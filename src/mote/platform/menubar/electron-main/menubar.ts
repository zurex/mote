...
import { mnemonicMenuLabel } from 'mote/base/common/labels';
import { isMacintosh, language } from 'mote/base/common/platform';
...
import { IMenubarData, IMenubarKeybinding, IMenubarMenu, IMenubarMenuRecentItemAction, isMenubarMenuItemAction, isMenubarMenuItemRecentAction, isMenubarMenuItemSeparator, isMenubarMenuItemSubmenu, MenubarMenuItem } from 'mote/platform/menubar/common/menubar';
...
import { IUpdateService, StateType } from 'mote/platform/update/common/update';
import { getTitleBarStyle, INativeRunActionInWindowRequest, INativeRunKeybindingInWindowRequest, IWindowOpenable } from 'mote/platform/window/common/window';
import { IWindowsCountChangedEvent, IWindowsMainService, OpenContext } from 'mote/platform/windows/electron-main/windows';
	...
	private runActionInRenderer(invocation: IMenuItemInvocation): void {
		...
		if (activeWindow) {
			...
			if (invocation.type === 'commandId') {
				...
				activeWindow.sendWhenReady('mote:runAction', CancellationToken.None, runActionPayload);
			} else {
				const runKeybindingPayload: INativeRunKeybindingInWindowRequest = { userSettingsLabel: invocation.userSettingsLabel };
				activeWindow.sendWhenReady('mote:runKeybinding', CancellationToken.None, runKeybindingPayload);
			}
		} else {
