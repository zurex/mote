...
import { SingleModifierChord, ResolvedKeybinding, ResolvedChord, Keybinding } from 'mote/base/common/keybindings';
...
import { IKeybindingService, IKeyboardEvent, KeybindingsSchemaContribution } from 'mote/platform/keybinding/common/keybinding';
import { IResolveResult, KeybindingResolver } from 'mote/platform/keybinding/common/keybindingResolver';
...
import { IME } from 'mote/base/common/ime';
...
export abstract class AbstractKeybindingService extends Disposable implements IKeybindingService {
	...
	public lookupKeybinding(commandId: string, context?: IContextKeyService): ResolvedKeybinding | undefined {
		const result = this.getResolver().lookupPrimaryKeybinding(commandId, context || this._contextKeyService);
		if (!result) {
			return undefined;
		}
	...
	public softDispatch(e: IKeyboardEvent, target: IContextKeyServiceTarget): IResolveResult | null {
		...
		return this.getResolver().resolve(contextValue, currentChord, firstChord);
	}
	...
	private _doDispatch(keybinding: ResolvedKeybinding, target: IContextKeyServiceTarget, isSingleModiferChord = false): boolean {
		...
		const resolveResult = this.getResolver().resolve(contextValue, currentChord, firstChord);
