...
import { IWindowsMainService } from 'mote/platform/windows/electron-main/windows';
import { defaultWindowUIState, IAppWindow, IWindowUIState, WindowMode } from 'mote/platform/window/electron-main/window';
import { isWorkspaceIdentifier, IWorkspaceIdentifier } from 'mote/platform/workspace/common/workspace';
...
export class WindowsStateHandler extends Disposable {
	...
	private onBeforeCloseWindow(window: IAppWindow): void {
	...
	private toWindowState(window: IAppWindow): IWindowState {
		return {
			windowId: window.id,
	...
	private doGetNewWindowState(configuration: INativeWindowConfiguration): INewWindowState {
		...
		let state = defaultWindowUIState();
