...
import { getDelayedChannel, ProxyChannel } from 'mote/base/parts/ipc/common/ipc';
import { AbstractUniversalWatcherClient, IDiskFileChange, ILogMessage, IRecursiveWatcher } from 'mote/platform/files/common/watcher';
	...
	protected override createWatcher(disposables: DisposableStore): IRecursiveWatcher {
		const watcher = ProxyChannel.toService<IRecursiveWatcher>(getDelayedChannel((async () => {
			...
			const { client, onDidTerminate } = await this.sharedProcessWorkerWorkbenchService.createWorker({
				moduleId: 'mote/platform/files/node/watcher/watcherMain',
				type: 'fileWatcher'
			});
