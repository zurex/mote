...
import { setProperty } from 'mote/base/common/jsonEdit';
import { Queue } from 'mote/base/common/async';
import { Edit, FormattingOptions } from 'mote/base/common/jsonFormatter';
import { Registry } from 'mote/platform/registry/common/platform';
import { IWorkspaceContextService, WorkbenchState } from 'mote/platform/workspace/common/workspace';
import { ITextFileService } from 'mote/workbench/services/textfile/common/textfiles';
import { IConfigurationService, IConfigurationUpdateOptions, IConfigurationUpdateOverrides } from 'mote/platform/configuration/common/configuration';
import { FOLDER_SETTINGS_PATH, WORKSPACE_STANDALONE_CONFIGURATIONS, TASKS_CONFIGURATION_KEY, LAUNCH_CONFIGURATION_KEY, USER_STANDALONE_CONFIGURATIONS, TASKS_DEFAULT, FOLDER_SCOPES } from 'mote/workbench/services/configuration/common/configuration';
import { FileOperationError, FileOperationResult, IFileService } from 'mote/platform/files/common/files';
import { IResolvedTextEditorModel, ITextModelService } from 'mote/editor/common/services/resolverService';
import { IConfigurationRegistry, ConfigurationExtensions, ConfigurationScope, keyFromOverrideIdentifiers, OVERRIDE_PROPERTY_REGEX } from 'mote/platform/configuration/common/configurationRegistry';
import { IEditorService } from 'mote/workbench/services/editor/common/editorService';
import { INotificationService, Severity } from 'mote/platform/notification/common/notification';
import { withUndefinedAsNull, withNullAsUndefined } from 'mote/base/common/types';
...
export class ConfigurationEditing {
	...
	private applyEditsToBuffer(edit: Edit, model: ITextModel): boolean {
		...
		const range = new EditorRange(startPosition.lineNumber, startPosition.column, endPosition.lineNumber, endPosition.column);
		const currentText = model.getValueInRange(range);
		if (edit.content !== currentText) {
			const editOperation = currentText ? EditOperation.replace(range, edit.content) : EditOperation.insert(startPosition, edit.content);
			model.pushEditOperations([new EditorSelection(startPosition.lineNumber, startPosition.column, startPosition.lineNumber, startPosition.column)], [editOperation], () => []);
			return true;
		}
		return false;
	}
