...
import { IFileService, FileOperationError, FileOperationResult, IFileStatWithMetadata, ICreateFileOptions, IFileStreamContent } from 'mote/platform/files/common/files';
import { Disposable } from 'mote/base/common/lifecycle';
import { extname as pathExtname } from 'mote/base/common/path';
...
import { VSBuffer, VSBufferReadable, bufferToStream, VSBufferReadableStream } from 'mote/base/common/buffer';
import { ITextSnapshot, ITextModel } from 'mote/editor/common/model';
...
import { IWorkspaceContextService, WORKSPACE_EXTENSION } from 'mote/platform/workspace/common/workspace';
...
import { consumeStream, ReadableStream } from 'mote/base/common/stream';
...
import { listErrorForeground } from 'mote/platform/theme/common/themeColors';
...
export abstract class AbstractTextFileService extends Disposable implements ITextFileService {
	...
	constructor(
		...
		@IMoteEditorService private readonly moteEditorService: IMoteEditorService,
	...
	private async doSaveAsTextFile(sourceModel: IResolvedTextEditorModel | ITextModel, source: URI, target: URI, options?: ITextFileSaveOptions): Promise<boolean> {
		...
		if (sourceTextModel && targetTextModel) {
			...
			const sourceTransientProperties = this.moteEditorService.getTransientModelProperties(sourceTextModel);
			if (sourceTransientProperties) {
				for (const [key, value] of sourceTransientProperties) {
					this.moteEditorService.setTransientModelProperty(targetTextModel, key, value);
				}
			}
		}
