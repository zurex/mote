...
import { isValidBasename } from 'mote/base/common/extpath';
import { Schemas } from 'mote/base/common/network';
import { IPath, win32, posix } from 'mote/base/common/path';
...
import { getVirtualWorkspaceScheme } from 'mote/platform/workspace/common/virtualWorkspace';
...
export abstract class AbstractPathService implements IPathService {
	...
	static findDefaultUriScheme(environmentService: IWorkbenchEnvironmentService, contextService: IWorkspaceContextService): string {
		...
		const firstPage = contextService.getWorkspace().pages[0];
		if (firstPage) {
			return firstPage.uri.scheme;
		}
