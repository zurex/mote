...
import { Cursor } from 'mote/editor/common/cursor/cursor';
import { CursorContext } from 'mote/editor/common/cursor/cursorContext';
import { CursorState, PartialCursorState } from 'mote/editor/common/cursorCommon';
import { compareBy } from 'mote/base/common/arrays';
...
export class CursorCollection {
	...
	*/
	...
	public readSelectionFromMarkers(): EditorSelection[] {
	...
	public getViewSelections(): EditorSelection[] {
	...
	public normalize(): void {
		...
		sortedCursors.sort(compareBy(s => s.selection, EditorRange.compareRangesUsingStarts));
		...
		for (let sortedCursorIndex = 0; sortedCursorIndex < sortedCursors.length - 1; sortedCursorIndex++) {
			...
			if (shouldMergeCursors) {
				...
				if (!looserSelection.equalsSelection(winnerSelection)) {
					...
					let resultingSelection: EditorSelection;
					if (resultingSelectionIsLTR) {
						resultingSelection = new EditorSelection(resultingRange.startLineNumber, resultingRange.startColumn, resultingRange.endLineNumber, resultingRange.endColumn);
					} else {
						resultingSelection = new EditorSelection(resultingRange.endLineNumber, resultingRange.endColumn, resultingRange.startLineNumber, resultingRange.startColumn);
					}
