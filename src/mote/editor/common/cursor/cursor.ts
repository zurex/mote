...
import { CursorState, ICursorSimpleModel, SelectionStartKind, SingleCursorState } from 'mote/editor/common/cursorCommon';
import { PositionAffinity, TrackedRangeStickiness } from 'mote/editor/common/model';
...
export class Cursor {
	...
	constructor(context: CursorContext) {
		...
		this._setState(
			context,
			new SingleCursorState(new EditorRange(1, 1, 1, 1), SelectionStartKind.Simple, 0, new Position(1, 1), 0),
			new SingleCursorState(new EditorRange(1, 1, 1, 1), SelectionStartKind.Simple, 0, new Position(1, 1), 0)
		);
	}
	...
	public readSelectionFromMarkers(context: CursorContext): EditorSelection {
		const range = context.model._getTrackedRange(this._selTrackedRange!)!;

		if (this.modelState.selection.isEmpty() && !range.isEmpty()) {
			// Avoid selecting text when recovering from markers
			return EditorSelection.fromRange(range.collapseToEnd(), this.modelState.selection.getDirection());
		}

		return EditorSelection.fromRange(range, this.modelState.selection.getDirection());
	}
	...
	private static _validateViewState(viewModel: ICursorSimpleModel, viewState: SingleCursorState): SingleCursorState {
		...
		return new SingleCursorState(
			EditorRange.fromPositions(validSStartPosition, validSEndPosition),
	...
	private _setState(context: CursorContext, modelState: SingleCursorState | null, viewState: SingleCursorState | null): void {
		...
		if (!viewState) {
			...
			const viewSelectionStart = new EditorRange(viewSelectionStart1.lineNumber, viewSelectionStart1.column, viewSelectionStart2.lineNumber, viewSelectionStart2.column);
