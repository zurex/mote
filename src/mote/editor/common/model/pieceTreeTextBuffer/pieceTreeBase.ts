...
import { FindMatch, ITextSnapshot, SearchData } from 'mote/editor/common/model';
import { NodeColor, SENTINEL, TreeNode, fixInsert, leftest, rbDelete, righttest, updateTreeMetadata } from 'mote/editor/common/model/pieceTreeTextBuffer/rbTreeBase';
import { Searcher, createFindMatch, isValidMatch } from 'mote/editor/common/model/textModelSearch';
...
export class PieceTreeBase {
	...
	public findMatchesInNode(node: TreeNode, searcher: Searcher, startLineNumber: number, startColumn: number, startCursor: BufferCursor, endCursor: BufferCursor, searchData: SearchData, captureMatches: boolean, limitResultCount: number, resultLen: number, result: FindMatch[]) {
		...
		do {
			...
			if (m) {
				...
				result[resultLen++] = createFindMatch(new EditorRange(startLineNumber + lineFeedCnt, retStartColumn, startLineNumber + lineFeedCnt, retEndColumn), m, captureMatches);
	...
	public findMatchesLineByLine(searchRange: EditorRange, searchData: SearchData, captureMatches: boolean, limitResultCount: number): FindMatch[] {
