...
import { ApplyEditsResult, EndOfLinePreference, FindMatch, IInternalModelContentChange, ISingleEditOperationIdentifier, ITextBuffer, ITextSnapshot, ValidAnnotatedEditOperation, IValidEditOperation, SearchData } from 'mote/editor/common/model';
import { PieceTreeBase, StringBuffer } from 'mote/editor/common/model/pieceTreeTextBuffer/pieceTreeBase';
import { countEOL, StringEOL } from 'mote/editor/common/core/eolCounter';
...
export class PieceTreeTextBuffer extends Disposable implements ITextBuffer {
	...
	public getRangeAt(start: number, length: number): EditorRange {
	...
	public getCharacterCountInRange(range: EditorRange, eol: EndOfLinePreference = EndOfLinePreference.TextDefined): number {
	...
	_toSingleEditOperation(operations: IValidatedEditOperation[]): IValidatedEditOperation {
		...
		const entireEditRange = new EditorRange(firstEditRange.startLineNumber, firstEditRange.startColumn, lastEditRange.endLineNumber, lastEditRange.endColumn);
		...
		for (let i = 0, len = operations.length; i < len; i++) {
			...
			// (1) -- Push old text
			result.push(this.getValueInRange(new EditorRange(lastEndLineNumber, lastEndColumn, range.startLineNumber, range.startColumn)));
	...
	findMatchesLineByLine(searchRange: EditorRange, searchData: SearchData, captureMatches: boolean, limitResultCount: number): FindMatch[] {
	...
	public static _getInverseEditRange(range: EditorRange, text: string) {
		...
		if (text.length > 0) {
			...
			if (lineCount === 1) {
				// single line insert
				resultRange = new EditorRange(startLineNumber, startColumn, startLineNumber, startColumn + firstLineLength);
			} else {
				// multi line insert
				resultRange = new EditorRange(startLineNumber, startColumn, startLineNumber + lineCount - 1, lastLineLength + 1);
			}
		} else {
	 ...
	 */
	public static _getInverseEditRanges(operations: IValidatedEditOperation[]): EditorRange[] {
		...
		for (let i = 0, len = operations.length; i < len; i++) {
			...
			if (op.text.length > 0) {
				...
				if (lineCount === 1) {
					// single line insert
					resultRange = new EditorRange(startLineNumber, startColumn, startLineNumber, startColumn + op.firstLineLength);
				} else {
					// multi line insert
	...
	private static _sortOpsAscending(a: IValidatedEditOperation, b: IValidatedEditOperation): number {
		const r = EditorRange.compareRangesUsingEnds(a.range, b.range);
		if (r === 0) {
