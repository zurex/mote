...
import { Client as MessagePortClient } from 'mote/base/parts/ipc/common/ipc.mp';
...
export async function connect(window: BrowserWindow): Promise<MessagePortMain> {
	...
	window.webContents.send('mote:createMessageChannel', nonce);

	// Wait until the window has returned the `MessagePort`
	// We need to filter by the `nonce` to ensure we listen
	// to the right response.
	const onMessageChannelResult = Event.fromNodeEventEmitter<{ nonce: string; port: MessagePortMain }>(validatedIpcMain, 'mote:createMessageChannelResult', (e: IpcMainEvent, nonce: string) => ({ nonce, port: e.ports[0] }));
