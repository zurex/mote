...
import { ProcessItem } from 'mote/base/common/processes';
	...
	return new Promise((resolve, reject) => {
		...
		if (process.platform === 'win32') {
			...
			function calculateLinuxCpuUsage() {
				...
				let cmd = JSON.stringify(FileAccess.asFileUri('mote/base/node/cpuUsage.sh').fsPath);
				cmd += ' ' + pids.join(' ');
			...
			exec('which ps', {}, (err, stdout, stderr) => {
				if (err || stderr) {
					if (process.platform !== 'linux') {
						...
						const cmd = JSON.stringify(FileAccess.asFileUri('mote/base/node/ps.sh').fsPath);
						exec(cmd, {}, (err, stdout, stderr) => {
							if (err || stderr) {
								reject(err || new Error(stderr.toString()));
							} else {
